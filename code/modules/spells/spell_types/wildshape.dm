...
/mob/living/carbon/human/species/wildshape/proc/gain_inherent_skills()
	if(mind)
		adjust_skillrank(/datum/skill/magic/holy, 3, TRUE) //Any dendorite using this should be a holy magic user

		var/datum/devotion/C = new /datum/devotion(src, patron) //If we don't do this, Dendorites can't be clerics and they can't revert back to their true forms
